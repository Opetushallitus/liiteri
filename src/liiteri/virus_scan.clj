(ns liiteri.virus-scan
  ...
  (:import [software.amazon.awssdk.services.sqs.model ReceiveMessageRequest GetQueueUrlRequest SendMessageRequest]
...
(defrecord VirusScanner [db storage-engine config]
  ...
  (start [this]
    (let [sqs-request-scan-client (get-sqs-client config)
          sqs-poll-results-client (get-sqs-client config)
